...
class XoopsTabs {
  ...
  function setCurrent( $name, $set='tabs' ) {
    if ( $set == 'tabs' ){$this->current_tab = $name;}
    if ( $set == 'subs' ){$this->current_sub = $name;}
  }
 ...
 */
 ...
 return $set;	
  ...
  function getStyle() {
    global $xoopsModule;
    return '<link rel="stylesheet" type="text/css" media="all" href="'.XOOPS_URL.'/modules/'.$xoopsModule->getInfo('dirname').'/css/tabs.css" />';
 }
}
